...
public class MyShapeManager {
	...
	public java0324.vo.MyShape findByPosition(int x1, int y1) throws ShapeNotFoundException {
...
//		return null; //못 찾았을 때 null 호출
		throw new ShapeNotFoundException(x1, y1); //사용자 정의 예외는 중급 레벨
	}

	
	public void drawAll() {
		System.out.println("**전체그리기**");
		for (MyShape myShape : arr) {
			System.out.println(myShape.draw()); //자식클래스의 모든 것을 불러온다
		}
	} //end drawAll()
